...
;; =============================
;; Features:
;; - 10 rows of seats, each row: 4 (left) • 12 (center) • 4 (right)
;; - Screen represented in green at the top
;; - Front and vertical aisles in blue
;; - Seat rows in black
;; - Outer walls in gray with 4 exits in red
;; =============================

;; Define patch variables
patches-own [
  blocked?   ;; true if patch is an obstacle (seat or wall)
  exit?      ;; true if patch is an exit
  wall?      ;; true if patch is a wall
]

;; Global variables for layout parameters and computed coordinates
globals [
  ;; seating parameters
  rows leftSeats centerSeats rightSeats
  verticalAisleWidth frontAisleDepth rowSpacing rowThickness

  ;; computed coordinates for convenience
...
;; =============================
to setup
  clear-all   ;; clear everything from previous runs

  ;; ----------------------------
  ;; PARAMETERS (user can change)
  ;; ----------------------------
  set rows 10                   ;; number of seat rows
  set leftSeats 4               ;; seats in left block
  set centerSeats 12            ;; seats in center block
  set rightSeats 4              ;; seats in right block
  set verticalAisleWidth 2      ;; width of vertical aisles separating L-C-R
  set frontAisleDepth 6         ;; depth of the front aisle in patches
  set rowSpacing 1              ;; vertical spacing between seat rows
  set rowThickness 1            ;; seat row thickness in patches

  ;; ----------------------------
  ;; COMPUTE WORLD SIZE
  ;; ----------------------------
  ;; total width of seating including aisles
  ...
  ;; compute world boundaries for NetLogo
  ...
  resize-world worldMinX worldMaxX worldMinY worldMaxY  ;; set world dimensions

  ;; ----------------------------
  ;; INITIALIZE PATCHES
  ;; ----------------------------
  ask patches [
    set pcolor white    ;; default floor color
    set blocked? false
    set exit? false
    set wall? false
  ]
  ...
  ;; ----------------------------
  ;; COMPUTE HORIZONTAL SEATING COORDINATES
  ;; ----------------------------
  set leftStartX (worldMinX + 1)                     ;; leftmost seat column
  ...
  set rightEndX worldMaxX - 1                         ;; seats touch right wall

  ;; ----------------------------
  ;; COMPUTE VERTICAL SEATING RANGE
  ;; ----------------------------
  set topSeatingY (worldMaxY - frontAisleDepth - 1)   ;; first row y-coordinate
  set bottomSeatingY (worldMinY + 5)                  ;; last row y-coordinate

  ;; ----------------------------
  ;; DRAW ALL COMPONENTS
  ;; ----------------------------
  draw-walls            ;; draw outer gray walls
  draw-screen           ;; draw green screen at top
  draw-front-aisle      ;; draw blue front aisle in front of first row
  draw-vertical-aisles  ;; draw blue vertical aisles separating seat blocks
  draw-seat-rows        ;; draw black seat rows
  draw-back-aisle       ;; draw blue aisle at the back
  carve-exits           ;; carve red exit patches
...
;; Draw outer walls (gray) around the edges
to draw-walls
  ask patches with [
...
;; Draw the screen (green) at the top
to draw-screen
  let screenHeight 1
  let screenY worldMaxY - 1    ;; one patch below top wall
  ask patches with [
...
;; Draw the front aisle (blue) between screen and first row
to draw-front-aisle
...
;; Draw vertical aisles separating left-center and center-right blocks
...
;; Draw all seat rows (black) block by block
to draw-seat-rows
  let i 0
  while [ i < rows ] [
    let y (topSeatingY - i * (rowThickness + rowSpacing))  ;; y-coordinate for current row
    ...
    set i (i + 1)   ;; move to next row
  ]
end

;; Draw the back aisle (blue) behind last row
to draw-back-aisle
  ask patches with [
...
;; Helper function: compute range of y-coordinates for an exit
...
;; Create exits at top and bottom for left and right walls
to carve-exits
  let exitHeight 2
  ...
  ;; create each exit using helper procedure
...
;; Helper: color patches red for exits and make them non-blocking
